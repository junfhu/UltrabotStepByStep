...
预期结果：全部 17 个测试通过（5 个熔断器 + 8 个提供者-模型映射 + 4 个自定义提供者）。要实际查看故障转移效果，可以在 `~/.ultrabot/config.json` 中配置两个提供者，
关闭主要提供者的 API，然后观察日志：

```
WARNING  Provider 'openai_compatible' failed: Connection refused. Trying next.
INFO     Falling back to provider 'anthropic' for model 'gpt-4o'
```
